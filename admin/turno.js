...
async function tomarTurno(event) {
    ...
    // Enviar notificación al barbero sobre nuevo cliente
    if (window.notificationSystem && window.notificationSystem.isNotificationEnabled()) {
        const { data: turnosEnEspera } = await supabase
            .from('turnos')
            .select('id', { count: 'exact', head: true })
            .eq('negocio_id', negocioId)
            .eq('fecha', hoy)
            .eq('estado', 'En espera');
        const queueLength = turnosEnEspera || 0;
        window.notificationSystem.notifyBarberNewClient(nombre, servicio, queueLength);
    }
...
async function atenderAhora() {
    ...
    if (window.notificationSystem && window.notificationSystem.isNotificationEnabled()) {
        const posicion = 1; // Es el turno actual
        window.notificationSystem.notifyClientTurn(turnoActual.nombre, posicion);
    }
...
async function guardarPago(event) {
    ...
    if (window.notificationSystem && window.notificationSystem.isNotificationEnabled()) {
        const { data: turnoData } = await supabase
            ...
            .single();
        if (turnoData) {
            window.notificationSystem.notifyServiceCompleted(turnoData.nombre, turnoData.servicio);
        }
    }
